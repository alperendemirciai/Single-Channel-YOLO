...
from ultralytics.utils import TIFF_NORM_CONST
...
class LoadStreams:
    ...
    def __init__(self, sources: str = "file.streams", vid_stride: int = 1, buffer: bool = False, channels: int = 3):
        ...
        self.cv2_flag = cv2.IMREAD_UNCHANGED if channels == 1 else cv2.IMREAD_COLOR  # grayscale or RGB
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            im = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)[..., None] if self.cv2_flag == cv2.IMREAD_UNCHANGED else im
        ...
        while self.running and cap.isOpened() and n < (f - 1):
            if len(self.imgs[i]) < 30:  # keep a <=30-image buffer
                ...
                if n % self.vid_stride == 0:
                    ...
                    im = (
                        cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)[..., None] if self.cv2_flag == cv2.IMREAD_UNCHANGED else im
                    )
                    if not success:
...
class LoadScreenshots:
    ...
    def __next__(self) -> Tuple[List[str], List[np.ndarray], List[str]]:
        ...
        im0 = cv2.cvtColor(im0, cv2.COLOR_BGR2GRAY)[..., None] if self.cv2_flag == cv2.IMREAD_UNCHANGED else im0
            ...
            if self.video_flag[self.count]:
                ...
                if success:
                    ...
                    im0 = (
                        ...
                        if self.cv2_flag == cv2.IMREAD_UNCHANGED
                        else im0
                    )
                    if success:
                        self.frame += 1
...
class LoadTensor:
    ...
    def _single_check(im: torch.Tensor, stride: int = 32) -> torch.Tensor:
        ...
        if im.max() > 1.0 + torch.finfo(im.dtype).eps:  # torch.float32 eps is 1.2e-07
            ...
            #LOGGER.warning(f"torch.Tensor inputs should be normalized 0.0-1.0 but max value is {im.dtype}. Dividing input by max({im.dtype}).")
            if im.max() <= 255.0 + torch.finfo(im.dtype).eps:
                im = im.float() / 255.0
            elif im.max() <= 65535.0 + torch.finfo(im.dtype).eps:
                im = im.float() / TIFF_NORM_CONST
            else:
                raise ValueError(
                    f"torch.Tensor inputs should be normalized 0.0-1.0 but max value is {im.max()}. "
                    f"Please normalize your input tensor before passing it to the model."
                )
