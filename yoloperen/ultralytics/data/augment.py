...
DEFAULT_MEAN_3CH = (0.0, 0.0, 0.0)
DEFAULT_STD_3CH = (1.0, 1.0, 1.0)
...
def classify_transforms(
    ...
    mean: Tuple[float, float, float] = DEFAULT_MEAN_3CH,
    std: Tuple[float, float, float] = DEFAULT_STD_3CH,
...
class ToTensor:
    ...
    def __call__(self, im: np.ndarray) -> torch.Tensor:
        ...
        #check if the image is uint8 or uint16 or float
        im = im.half() if self.half else im.float()  # uint8 to fp16/32
        im_max = im.max()

        if im_max <= 1.0 + torch.finfo(torch.float32).eps:
            pass # already between 0.0 and 1.0
        elif im_max <= 255.0 + torch.finfo(torch.float32).eps:
            im /= 255.0
        elif im_max <= 65535.0 + torch.finfo(torch.float32).eps:
            im /= TIFF_NORM_CONST
        else:
            raise ValueError(f"Invalid image max value {im_max}, expected <= 65535.0 for uint16 or <= 255.0 for uint8.")
