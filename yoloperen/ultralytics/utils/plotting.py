...
def plot_images(
    ...
    if images.shape[1] == 1:  # grayscale
        ...
        images = images[:, :3]  # convert to RGB
    
    """
    
    print("Image shape | images.shape | bs | ns | h | w ")  # debug info
    print(images[0].shape, images.shape, bs, ns, h, w)  # debug info

    print("Image[0].max | Image[0].min | Image[0].mean | Image[0].std")  # debug info
    print(images[0].max(), images[0].min(), images[0].mean(), images[0].std())  # debug info
